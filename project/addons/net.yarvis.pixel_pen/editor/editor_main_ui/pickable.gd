...
func _on_pressed():
	...
	if (owner as Control).get_parent().has_method("_on_pickable_pressed"):
		...
		(owner as Control).get_parent()._on_pickable_pressed(mouse, (owner as Control).layer_uuid, 0)
...
func _on_released():
	...
	if (owner as Control).get_parent().has_method("_on_pickable_pressed"):
		...
		(owner as Control).get_parent()._on_pickable_pressed(mouse, (owner as Control).layer_uuid, 2)
...
func _process(delta):
	...
	if is_pressed:
		...
		if (owner as Control).get_parent().has_method("_on_pickable_pressed"):
			(owner as Control).get_parent()._on_pickable_pressed(mouse, (owner as Control).layer_uuid, 1)
